...
let mapMenuEntry node =
    ...
    | headerNode::priceNode::headerNode2::priceNode2::_ ->
        ...
        ;
            ...
            | Some (number, name) -> Some { Number = number; Name = name.ToLower(); Price = parsePrice priceNode2 }
            | None -> None)
        ] |> Seq.choose id |> Seq.toList
    | headerNode::priceNode::_ ->
        match mapMenuItemHeader headerNode with
        | Some (number, name) -> [ { Number = number; Name = name.ToLower(); Price = parsePrice priceNode } ] |> Seq.toList
        | None -> [] |> Seq.toList
    | _ -> [] |> Seq.toList
